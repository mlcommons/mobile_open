...
TENSOR_NAMES_WITH_SEPARATE_LM_HEAD = model_builder.TENSOR_NAMES_WITH_SEPARATE_LM_HEAD
...
def get_8b_model_config() -> cfg.ModelConfig:
  ...
  ff_config = cfg.FeedForwardConfig(
      ...
      activation=cfg.ActivationConfig(
          cfg.ActivationType.SILU
      ),
      intermediate_size=14336,
      use_bias=False,
  )

  pre_norm = cfg.NormalizationConfig(
  ...
  block_config = cfg.TransformerBlockConfig(
      attn_config=attn_config,
      ff_config=ff_config,
      pre_attention_norm_config=pre_norm,
      post_attention_norm_config=pre_norm,
...
def _build_8b_model(
  ...
  return model_builder.build_decoder_only_model(
      checkpoint_path=checkpoint_path,
      config=config,
      tensor_names=TENSOR_NAMES_WITH_SEPARATE_LM_HEAD,
      model_class=Llama,
      custom_loader=custom_loader,
      mask_cache_size=mask_cache_size,
  )
