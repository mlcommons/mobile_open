...
|MobileNetV4 - Single stream |LoadGen sends next query as soon as SUT completes the previous query | 1024 |1024 |60 sec |90% | 90%-ile measured latency
|MobileNetV4 - Offline |LoadGen sends all queries to the SUT at start | 1024 | 24,576 |None |N/A | Measured throughput
...
|Stable Diffusion V1.5^ - Single stream |LoadGen sends next query as soon as SUT completes the previous query | 1024 |1024 |60 sec |90% | 90%-ile measured latency
|Llama 3.1 8B^^ - Single stream |LoadGen sends next query as soon as SUT completes the previous query | 100 |1024 |60 sec |mean | mean TFT / mean TPS 
|Llama 3.2 3B^^^ - Single stream |LoadGen sends next query as soon as SUT completes the previous query | 100 |1024 |60 sec |mean | mean TFT / mean TPS 
|===
^   Optional Benchmark, SDv1.5 run with 20 UNET iterations
^^  Optional Benchmark: Run for both LLM (TInyMMLU-FewShot2) Benchmark and LLM-Instruct (IFEval-33) benchmark, TFT: Time to First Token, TPS: Token Per Second
^^^ Optional Benchmark: Run for both LLM (TInyMMLU-FewShot2) Benchmark and LLM-Instruct (IFEval-33) benchmark 
...
|MobileNetV4 - Single stream |ImageNet 2012 validation data set (50000 images) | http://image-net.org/challenges/LSVRC/2012/ | 81% ~ 98% of FP32 (Top 1: 82.68%) 
|MobileNetV4 - Offline |ImageNet 2012 validation data set (50000 images) | http://image-net.org/challenges/LSVRC/2012/ | 81% ~ 98% of FP32 (Top 1: 82.68%) 
|MobileDet-SSD - Single stream |MS-COCO 2017 validation set (5000 images) | http://images.cocodataset.org/zips/val2017.zip | 27.075% ~ 95% of FP32 (mAp: 28.5%)
|MOSAIC - Single stream |ADE20K val set (2000 images) | http://data.csail.mit.edu/places/ADEchallenge/ADEChallengeData2016.zip | 57.4% ~ 96% of FP32 (32-class mIOU: 59.8%)
|EDSR - Single stream |Selected Google Open Image (25 images) | https://github.com/mlcommons/mobile_models/blob/main/v3_0/datasets/snusr_lr.zip | 33 dB PSNR (FP32: 33.58 dB) 
|MobileBERT - Single stream |SQUAD v1.1 Dev (dev-v1.1.json) (10833 samples)  * Mini-validation set with 100 samples is adopted by MWG | https://github.com/google-research/bert#squad-11 | 87.4% ~ 93% of FP32 (F1-score 94%) 
|Stable Diffusion V1.5 - Single stream | MS-COCO 2014 captions| https://huggingface.co/datasets/justram/COCO2014-Captions| 25 CLIP Score (FP16 CLIP Score 25.31)
|Llama 3.1 8B - Single stream |TInyMMLU and Tiny-IFEval-33 |  https://huggingface.co/datasets/tinyBenchmarks/tinyMMLU & [IFEval33](/dataset/ifeval_33.jsonl)|TBD
|Llama 3.2 3B - Single stream |TInyMMLU and Tiny-IFEval-33 |  https://huggingface.co/datasets/tinyBenchmarks/tinyMMLU & [IFEval33](/dataset/ifeval_33.jsonl)|TBD
|===
== Benchmarks
...
*** <Benchmark>
**** Calibration_process.adoc (Vendor model files and/or instructions to generate them)
*** <Runtime>/
**** Instructions to build apk with vendor backend and <git commit from the mobile_app_closed repo> 
**** Additional instructions and code
...
*** result.json (Summary result file copied directly from DUT: \Internal storage\Android\data\org.mlcommons.android.mlperfbench\files\results)
***YYYY-MM-DDTHH-MM-SS.SSSSS (Results Log folder copied directly from DUT: \Internal storage\Android\data\org.mlcommons.android.mlperfbench\files\logs)
**** <scenario>
...
<**benchmark**> must be one of {**MobilenetV4, MobiledEtSSD, MOSAIC, EDSR, MobileBERT, stableDiffusion, LLama3p1_8B, Llama3p2_3B**}. 
<**scenario**> must be one of { **SingleStream, Offline**}.

Multiple results folders and result.json files, corresponding to different Submission Runs can be submitted: 
*Gen-AI models (Stable-Diffusion, Llama 3.1 8B, Llama 3.2 3B Submission runs may be performed in separate Submission Runs in MLPerf App, hence may have their own results folder and result.json file)
*Other models must be run in a Single Submission run, and submitted in the same results folder, and in same result.json file as generated by the app.
...
Vendor’s glue code which interfaces with Mlperf app frontend must be commited in mlperf_app_closed 
