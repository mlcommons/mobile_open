= MLPerf Mobile v1.1 Submission Process and Auditing Protocols
...
. Load inference data set(s) -- do not use the dummy dataset! 
...
* Submitter will be required to provide both the inference and accuracy results for all 4 models under single-stream scenario and MobilenetEdgeTPU offline scenario directly from the UI of the mlperf app compiled by the submitter
...
*<Benchmark name>* = {mobilenetEdgeTPU, mobiledetSSD, mobileBERT, deeplabV3+} +
...
* MLCommon (David Kanter) will take each vendor’s submission code (with instruction), generate the vendor-specific backend, and integrate with the MLperf app front end (and vendor’s SDK, if applicable) to create a vendor specific mlperf app for each submitter. These apps will be upstreamed to MLCommon github for each submitter to reproduce other submitter’s results
* Submitters can try to reproduce other submitter’s claimed results with the vendor-specific mlperf app provided by MLcommon. If a submitter has issue on obtaining another submitter’s device under test, MLCommon (David Kanter) can test on submitter’s behalf
** The procedure for reproducing result should be same as mentioned in Result Submission Procedure
