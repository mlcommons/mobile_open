...
def _create_input_tensors():
  ...
  input_image = tf.placeholder(tf.float32, [1, 512, 512, 3], name=_INPUT_NAME)
  original_image_size = tf.shape(input_image)[1:3]
  
  resized_image_size = tf.TensorShape([512,512])
  image = input_image
...
def main(unused_argv):
  ...
  with tf.Graph().as_default():
    ...
    output_graph_def = tf.compat.v1.graph_util.convert_variables_to_constants(
        sess, graph_def, ['ArgMax']
    )

    tf.train.write_graph(output_graph_def, FLAGS.export_path, 'freeze.pb', as_text=False)
