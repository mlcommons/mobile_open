...
def multi_scale_logits(images,
  ...
  logits_height = scale_dimension(
      crop_height, 1.0 / logits_output_stride)
  logits_width = scale_dimension(
      crop_width, 1.0 / logits_output_stride)
...
def extract_features(images,
                     ...
                     fine_tune_batch_norm=False,
                     nas_training_hyper_parameters=None):
  """Extracts features by the particular.
      ...
      with tf.variable_scope(DECODER_SCOPE, DECODER_SCOPE, [features]):
        ...
        for output_stride in decoder_output_stride:
          ...
          for i, name in enumerate(feature_list):
            ...
            decoder_features_list[0] = _resize_bilinear(
              decoder_features_list[0], [decoder_height, decoder_width], decoder_features_list[0].dtype)
...
def get_branch_logits(features,
  ...
  if atrous_rates is None:
    if kernel_size != 1:
      raise ValueError('Kernel size must be 1 when atrous_rates is None. '
                       'Gets %d.' % kernel_size)
    atrous_rates = [1]
